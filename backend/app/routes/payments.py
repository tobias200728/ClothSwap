...
async def process_payment(
    ...
    if data.plan_id != "free" and data.payment_method in ("mastercard", "visa"):
    ...
    supabase.table("users").update({"plan": data.plan_id}).eq("id", current_user["id"]).execute()

    plan_names = {"free": "Free", "basic": "Basic", "premium": "Premium"}
